...
3. **Build** — must pass with 0 warnings before committing:
   ...
   az deployment sub what-if --location australiaeast --template-file infra/main.bicep --parameters infra/main.parameters.dev.bicepparam --parameters postgresAdminPassword=Placeholder123!   # infra (if changed); postgresAdminPassword is omitted from the params file intentionally — supply a placeholder for local validation only
   ```
